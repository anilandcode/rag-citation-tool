...
def cmd_ingest(args):
    ...
    print(f"Ingested {len(nodes)} chunks from {len({n.metadata.get('source') for n in nodes})} documents")


def cmd_query(args):
    nodes = run_ingestion(input_dir=args.input_dir)
    _, hybrid_retriever, reranker = build_full_retrieval_pipeline(nodes)
    qe = build_query_engine(retriever=hybrid_retriever, node_postprocessors=[reranker])
...
def cmd_verify(args):
    ...
    print(f"Citations: {report.total_citations}, Verified: {report.verified}, Accuracy: {report.accuracy:.2%}")
...
def main():
    ...
    eval_parser = subparsers.add_parser("eval", help="Run evaluation on a golden dataset")
